...
public class GroupMessengerActivity extends Activity {
	...
	private int overallSequenceNumber = 0; // Used by Sequencer to allocated new
											// sequence
	private int counter = 0; // At each process concatenate with the port number
								// & increment this
	...
	private Hashtable<Integer, String> msgWaitingQueue = new Hashtable<Integer, String>(); // Initially
																							// when
																							// messages
																							// are
																							// coming
	...
	private PriorityQueue<OrderQueue> orderQueue; // When displaying message
													// check on this priority
													// Queue which is
													// prioritised (ascending)
	...
	/*
	 * This thread class responsible for receiving messages.
	 */
	...
	/*
	 * This thread responsible for creating the object and sending to the
	 * socket.
	 */
	public class ServingClient implements Runnable {
		...
		public void run() {
			try {
				...
				Executors.newSingleThreadExecutor().execute(
						new ProcessMessageQueue(receive));
	...
	public void testCase2(View view) {
		...
		Message m = new Message(Integer.parseInt(id), vectorClock, "message",
				currAvd + ":" + testcase2_num, portStr, true);
		++counter;
		++testcase2_num;
		for (int i = 0; i < 3; ++i)
	...
	/*
	 * This class will process the message & check the message If the message is
	 * from sequencer then match it with process group id else put the message
	 * into waiting queue
	 */
	public class ProcessMessageQueue implements Runnable {
		...
		public void processMessage(Message msgRecv) {
			if ((msgRecv.msgType).equals("Order")) { // First put the order
														// message in queue &
														// then check to avoid
														// simultaneous action
														// of process thread &
				...
				Log.e("vector No",
						Integer.toString(msgRecv.vecClock[0]) + ":"
								+ Integer.toString(msgRecv.vecClock[1]) + ":"
								+ Integer.toString(msgRecv.vecClock[2]));
				if (portStr.equalsIgnoreCase("5556")) {
					boolean flag = false;
					if ((msgRecv.vecClock[msgRecv.v_id] == vectorClockProcess[msgRecv.v_id] + 1)) { // &&
																									// (msgRecv.vecClock[i]
																									// <=
																									// vectorClockProcess[i]))
																									// {
						flag = true;
					} else {
						flag = false;
					...
					if (flag) {
						vectorClockProcess[msgRecv.v_id] = Math.max(
								vectorClockProcess[msgRecv.v_id],
								msgRecv.vecClock[msgRecv.v_id]);
						Executors.newSingleThreadExecutor().execute(
								new Sequencer(msgRecv));
					} else {
					...
					if (globalEvent.size() > 0) {
						...
						while (it.hasNext()) {
							...
							boolean f = false;
							if (first.vecClock[first.v_id] == vectorClockProcess[first.v_id] + 1) { // &&
																									// (first.vecClock[i]
																									// <=
																									// vectorClockProcess[i]))
																									// {
							...
							if (f) {
								vectorClockProcess[first.v_id] = Math.max(
										vectorClockProcess[first.v_id],
										first.vecClock[first.v_id]);
								Executors.newSingleThreadExecutor().execute(
										new Sequencer(first));
								it.remove();
							}
					...
					for (int i = 0; i <= 2; ++i) {
						vectorClock[i] = Math.max(vectorClock[i],
								msgRecv.vecClock[i]);
					}
				}
			}
			while (orderQueue.size() != 0) {
				...
				if (front.newId == groupSequenceNumber
						&& msgWaitingQueue.containsKey(front.oldId)) {
					Log.e("Inside msg waitig group",
							Integer.toString(groupSequenceNumber));
					final String msg = msgWaitingQueue.get(front.oldId); // split[1]
																			// here
																			// contains
																			// msg
																			// key
					msgWaitingQueue.remove(front.oldId); // Remove this and
															// insert same msg
															// with new key sent
															// by sequencer
					groupSequenceNumber += 1; // Increment group Sequencer
												// Number because msg is
		...
		public void processTestCase2(Message msgRecv) {
			if (msgRecv.first) {
				...
				while (j < 3) {
					String id = (portStr) + Integer.toString(counter);
					++vectorClock[currIndex];
					Message m = new Message(Integer.parseInt(id), vectorClock,
							"message", currAvd + ":"
									+ Integer.toString(testcase2_num), portStr,
							false);
					++counter;
					++testcase2_num;
					for (int i = 0; i < 3; ++i)
	...
	/*
	 * This class work as a Sequencer & follow FIFO ordering(array is used for
	 * each index denotes a process) when messages are coming for sequences
	 */
	public class Sequencer implements Runnable {
		...
		public void processSequencer(Message msgRecv) {
			Message m = new Message(msgRecv.msgId, overallSequenceNumber,
					"Order", portStr);
	...
	/*
	 * Test Case 1 class: Responsible for Multicasting 5 message with a sleep
	 * time of 3 seconds
	 */
	...
	/*
	 * OrderQueue Class is used to create a priority queue used to store order
	 * message coming from sequencer Below this class is comparator defined for
	 * the queue
	 */
